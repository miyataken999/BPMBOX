...
class RunnerPage
{
	...
	function getMasterRecord()
	{
		...
		$strGroupAndHaving = "";
		$strGroup = $masterQuery->GroupByToSql();
		$strHaving = $masterQuery->HavingToSql();
		if ( strlen($strGroup) )
			$strGroupAndHaving .= $strGroup;
		if ( strlen($strHaving) )
			$strGroupAndHaving .= " HAVING ". $strHaving;

		$strSQL = $masterQuery->HeadToSql().' '.$masterQuery->FromToSql().$strWhere.$strGroupAndHaving;
...
//		foreach ($this->googleMapCfg['mainMapIds'] as $mapId)
	 ...
	 * @return Array
	 */
	protected function getOrderClauseFieldsListData()
	{
		require_once(getabspath('classes/orderclause.php'));
	
		$orderClause = new OrderClause( $this, $_SESSION[ $this->sessionPrefix."_sortby" ] );
		...
		return $orderClause->getOrderClauseFieldsData();				
	}
	...
	function getNextPrevRecordKeys(&$data, $securityMode, &$next, &$prev, $onDash = false)
	{
		...
		$fieldsList = $this->getOrderClauseFieldsListData();
		if( !count($fieldsList) )
		{
...
//		$this->updateActualListPageNumber( $whereData["where_prev"], $SQLData["sql_prev"] );
	...
	{
		if( $this->masterTable ) //view edit on dpreview
			$where = $this->getMasterTableSQLClause( true );
		else
			$where = $_SESSION[$this->sessionPrefix."_where"];
	
		if( $onDash && !$this->masterTable && !$this->searchClauseObj->bIsUsedSrch )
			$where = "";
		...
		if( $onDash && $this->searchClauseObj->bIsUsedSrch )
		{
			...
			$where = whereAdd( $where, SecuritySQL( $securityMode, $this->tName ) );
	 ...
	 * @param Array orderFieldsData				The key and orderby fields
	 * @param String option (optional)
	 * @return Array
	 */
	function getSQLPrevNextOrederClauses( $orderFieldsData, $option = "both" )
	{
		...
		for($i = 0; $i < count($orderFieldsData); $i++)
		{
			$field = $orderFieldsData[$i];
	 ...
	 * @return Array
	 */
	function getSQLPrevNextWhereClauses( $orderFieldsData, $data, $hasGroupBy, $option = "both" )
	{		
		if( $option != "both" && $option != "next" && $option != "prev")
			$option = "both";
		...
		for($i = 0; $i < count($orderFieldsData); $i++)
		{
			...
			if( $isNext	)
				$where_next.= (strlen( $where_next ) > 0 ? " AND " : "")."(".$nextExpr;
			if( $isPrev )
				$where_prev.= (strlen( $where_prev ) > 0 ? " AND " : "")."(".$prevExpr;
	...
	protected function getOrderByClause()
	{
		...
		return $orderClause->buildOrderParams();
		/*$orderByList = array();
		foreach( $orderClause->getOrderFieldsDataForAttrs() as $fData )
		{
			$fieldName = $this->pSet->GetFieldByIndex( $fData["fieldIndex"] );
			if( !$fieldName )
				continue;
			...
			$orderByList[] = $fullName." ".$fData["orderDirection"];		
		}
	...
	function buildPagination() 
	{
		...
		else
		{
			...
			if($this->maxPages > 1) 
			{
				...
				if($counterstart != 1) 
				{
					$pagination.= $this->getPaginationLink(1,"先頭") . $advSeparator;
					$pagination.= $this->getPaginationLink($counterstart - 1,"前").$separator;
				}
				$pageLinks = "";
				...
				if($counterend != $this->maxPages) 
				{
					$pagination.= $separator . $this->getPaginationLink($counterend + 1,"次") . $advSeparator;
					$pagination.= $this->getPaginationLink($this->maxPages,"最後");
				}
				if( $this->getLayoutVersion() == BOOTSTRAP_LAYOUT )
		...
		foreach($fieldsData as $fieldName => $value)
		{
			...
			if($this->hasDuplicateValue($fieldName, $value)) 
			{
				...
				if($this->mode != EDIT_POPUP && $this->mode != ADD_POPUP)
					$message = $fieldName." "."フィールドの値が重複しています";
	...
	*/
	...
	function getProceedLink() 
	{
		...
		return '<span class="rnr-dbebrick">'
			...
			.  "進む" . ' '. GetTableCaption( GoodFieldName( $this->tName ) ) 
			. '</a>' 
			. "&nbsp;&nbsp;</span>";
	}
	...
	{
		if( $page == "add" )
			return GetTableCaption($table).", "."追加";
		if( $page == "edit" )
			return GetTableCaption($table).", "."編集"." [". RunnerPage::getKeysTitleTemplate( $table, $pSet ). "]";
		if( $page == "view" )
			return GetTableCaption($table)." [". RunnerPage::getKeysTitleTemplate( $table, $pSet ). "]";
		if( $page == "export" )
			return "エクスポート";
		if( $page == "import" )
			return GetTableCaption($table).", "."インポート";
		if( $page == "search" )
			return GetTableCaption($table)." - "."詳細検索";
		if( $page == "print" )
	...
	function countTotals(&$totals, &$data) 
	{
		for($i = 0; $i < count($this->totalsFields); $i ++)
		{
			...
			{
				if ( $this->totalsFields[$i]['viewFormat'] == FORMAT_CHECKBOX && intval($data[$this->totalsFields[$i]['fName']]) === 0 )
				{
					continue;
				}
