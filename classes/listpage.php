...
{	
	...
	function __construct(&$params) 
	{
		...
		$this->orderClause = new OrderClause( $this, $this->sortBy, $this->pSet->useMoveNext() );
	}
	...
	{
		$urlList = $this->orderClause->getOrderByParams();
		...
		if ( count($urlList) > 0 )
		{
			...
			$this->controlsMap["urlParams"]["orderby"] = implode(';', $urlList);
		}
	}
	...
	function addCommonHtml() 
	{
		...
		if($this->is508) {
			$this->body["begin"].= "<a href=\"#skipdata\" title=\""."テーブルデータに飛ぶ"."\" class=\"".$this->makeClassName("s508")."\">"."テーブルデータに飛ぶ"."</a>";
			$this->body["begin"].= "<a href=\"#skipmenu\" title=\""."メニューに飛ぶ"."\" class=\"".$this->makeClassName("s508")."\">"."メニューに飛ぶ"."</a>";
			$this->body["begin"].= "<a href=\"#skipsearch\" title=\""."検索に飛ぶ"."\" class=\"".$this->makeClassName("s508")."\">"."検索に飛ぶ"."</a>";
			$this->body["begin"].= "<a href=\"templates/helpshortcut.htm\" title=\""."ホットキーの参照"."\" class=\"".$this->makeClassName("s508")."\">"."ホットキーの参照"."</a>";
		}
	...
	function setSessionVariables() 
	{
		...
		if( @$_REQUEST["orderby"] )
		{
			...
			unset( $_SESSION[ $this->sessionPrefix."_sortby" ] );
		}
		...
		if( @$_REQUEST["sortby"] )
			$_SESSION[ $this->sessionPrefix."_sortby" ] = @$_REQUEST["sortby"];
		
		if( !$this->sortBy && $_SESSION[ $this->sessionPrefix."_sortby" ] )
			$this->sortBy = $_SESSION[ $this->sessionPrefix."_sortby" ];
	...
	function orderLinksAttr()
	{
		for($i = 0; $i < count($this->listFields); $i++) 
		{
			$goodName = GoodFieldname( $this->listFields[$i]['fName'] );
			$this->xt->assign( $goodName."_orderlinkattrs", $this->setLinksAttr( $goodName ) );
		...
		foreach($this->selectedRecs as $keys) 
		{
			...
			if(!$lockRecord && @$_REQUEST["a"] == "delete" && $retval) 
			{
						...
						if( $this->tName == ADMIN_USERS )
				{
					...
					$sql = "delete from ". $grConnection->addTableWrappers( "processmaker_ugmembers" ) 
	...
	protected function seekPageInRecSet( $strSQL ) 
	{
		...
		{
			$orderFieldsData = $this->orderClause->getOrderFieldsData();
	...
	function buildSQL() 
	{
		...
		{		
			if( $whereModifiedInEvent ) 
				$strSQL = SQLQuery::gSQLWhere_having($this->gsqlHead, $sqlFrom, $sqlWhere, $this->gsqlGroupBy, $this->gsqlHaving, $this->strWhereClause, $this->strHavingClause, $searchCombineType);	
			else
				$strSQL = SQLQuery::gSQLWhere_having($this->gsqlHead, $sqlFrom, $sqlWhere, $this->gsqlGroupBy, $sqlHaving, $searchWhere, $searchHaving, $searchCombineType);	
	...
	{	
		if( $this->inlineEditAvailable() || $this->displayViewLink() || $this->editAvailable() )
		{
	...
	function buildOrderParams()
	{
		...
		$this->strOrderBy = $this->orderClause->buildOrderParams();		
			
		$this->buildOderFieldLabels();
	 ...
	 */
	protected function buildOderFieldLabels()
	{
		if( !$this->isReoderByHeaderClickingEnabled() )	
			return;
		
		foreach( $this->orderClause->getOrderFieldsDataForAttrs() as $fData )
		{
			$oFName = GoodFieldName( $this->pSet->GetFieldByIndex( $fData["fieldIndex"] ) );
			$oFDir = $fData["orderDirection"] == "ASC" ? "a" : "d";
			
			if( $fData["setIcon"] )
				$this->xt->assign_section( $oFName."_fieldheader", "", "<span data-icon=\"".($oFDir == "a" ? "sortasc" : "sortdesc")."\"></span>" );
				
			// default ASC for key fields	
			$orderLinkAttrs = $this->setLinksAttr( $oFName, $oFDir, $fData["setIcon"] );		
			$this->xt->assign( $oFName."_orderlinkattrs", $orderLinkAttrs );							
		}	
	}
	...
	protected function assignSortByDropdown()
	{
		...
		$sortSettings = $this->orderClause->getSortByControlSettings();		
		$sortByIdx = $this->orderClause->getSortByIdxAdjusted();
