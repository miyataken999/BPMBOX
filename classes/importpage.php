...
//		$importFields = $this->pSet->getImportFields();
	...
	protected function getBasicReportText( $totalRecords, $addedRecords, $updatedRecords, $isNotLogFile = true, $lineBreak = "<br>", $errorMessages = array(), $unprocessedData = array() )
	{
		...
		{
			$reportText .= "インポート先"." ".$this->strOriginalTableName.$lineBreak.
				str_format_datetime( db2time( now() ) ) .$lineBreak.$lineBreak;
		}
		
		$reportText .= mysprintf("%s2 レコードのうち %s1 を処理しました。  // alternative :: %s/%s レコードを処理しました。", array($boldBegin.$importedReords.$boldEnd, $boldBegin.$totalRecords.$boldEnd)) .$lineBreak.
			mysprintf("%s レコードを追加しました。", array($boldBegin.$addedRecords.$boldEnd)) .$lineBreak.
			mysprintf("%s レコードを更新しました。", array($boldBegin.$updatedRecords.$boldEnd)) .$lineBreak;
			
		if( $notImportedRecords )
			$reportText.= mysprintf("%s レコードでエラーがありました。", array($boldBegin.$notImportedRecords.$boldEnd));
