...
class UploadHandler
{
    ...
    public function validate($uploadedFile, &$file, $error, $file_size, $index, $uploadDir) {
        ...
        if (!$file["name"]) {
            $file["error"] = "ファイル名が指定されていません";
            return false;
        }
        if (!preg_match($this->options['accept_file_types'], $file["name"])) {
            $file["error"] = "ファイル型が不正です";
            return false;
        }
        if ($this->options['max_file_size'] && (
            ...
            $file["error"] = mysprintf("ファイルの大きさが制限値(%s キロバイト)を超えました", array($this->options['max_file_size']));
            return false;
        }
        if ($this->options['min_file_size'] &&
            $file_size < $this->options['min_file_size'] * 1024) {
            $file["error"] = mysprintf("ファイルの大きさは %s キロバイト以上にしてください", array($this->options['min_file_size']));
            return false;
        }
   		if (is_int($this->options['max_totalFile_size']) && (
                $this->getUploadFilesSize() + $file["size"] > $this->options['max_totalFile_size'] * 1024)
            ) {
            $file["error"] = mysprintf("ファイルの合計サイズが制限(%s キロバイト)を超えました", array($this->options['max_totalFile_size']));
            return false;
        }
        if (is_int($this->options['max_number_of_files']) && (
            	...
            	if($this->options['max_number_of_files'] > 1)
            		$file["error"] = mysprintf("アップロードできるのは %s 個までです", array($this->options['max_number_of_files']));
            	else 
            		$file["error"] = "アップロードできるファイルは一つだけです";
            return false;
        }
    ...
    public function handle_file_upload($uploadedFile, $index) {
		...
		if($this->pSet->isMakeDirectoryNeeded($this->field))
		{
			...
			{
				$file["error"] = "アップロード対象のフォルダがありません";
				return $file;
			}
		}
