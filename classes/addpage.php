...
class AddPage extends RunnerPage
{
	...
	protected function recheckUserPermissions()
	{
		...
		if( isLoggedAsGuest() || !isLogged() ) 
		{
			$this->setMessage( "セッションが失効しました。" 
				. "<a href='#' id='loginButtonContinue" . $this->id . "'>" . "ログインして" . "</a>" 
				. "データを保存。" );
		}
		else
		{
	...
	protected function setSuccessfulUpdateMessage()
	{
		...
		if( $this->mode == ADD_INLINE ) 
			$infoMessage = ""."レコードが追加されました。"."";
		else
			$infoMessage = "<strong><<< "."レコードが追加されました。"." >>></strong>";
		...
		else 
		{
			...
			if( $this->editAvailable() )
				$infoMessage.= "&nbsp;<a href='".GetTableLink( $this->pSet->getShortTableName(), "edit", $keylink )."'>"."編集"."</a>&nbsp;";
				
			if( $this->viewAvailable() )
				$infoMessage.= "&nbsp;<a href='".GetTableLink( $this->pSet->getShortTableName(), "view", $keylink )."'>"."表示"."</a>&nbsp;";
		}
	...
	public function setDatabaseError( $message )
	{
		if( $this->mode != ADD_INLINE )
		{
			$this->message = "<strong>&lt;&lt;&lt; "."レコードは追加されませんでした"."</strong> &gt;&gt;&gt;<br><br>".$message;
		}
		else
		{
			$this->message = "レコードは追加されませんでした".". ".$message;
		}
