...
class RemindPasswordPage extends RunnerPage
{
	...
	{		
		...
		if( !$tosearch || !$data )
		{
			$this->message = "ユーザ "." ".$this->userEmail." "."は、登録されていません。";
			return false;
		}
	...
	protected function sendRemindEmail( $username, $password, $email, $token, $dbPassword )
	{
		...
		$sentMailResults = runner_mail( array("to" => $email, "subject" => "パスワードリマインダ", "body" => $message ) );
	...
	protected function getRemindEmailMessage( $username, $password, $token, $dbPassword )
	{
		...
		$message.= "パスワードリマインダ"."\r\n";
		$message.= "あなたは次の場所でユーザ名とパスワードを送付するよう要求されました。"." ".$loginUrl."\r\n";
		$message.= "ユーザ名".": ".$username."\r\n";
