...
class EditPage extends RunnerPage
{
	...
	public function getWhereClause( $useOldKeys )
	{
		...
		elseif( $this->mode == EDIT_DASHBOARD )
		{
			...
			if( $this->masterTable ) // filter by master
				$strWhereClause = whereAdd( $strWhereClause, $this->getMasterTableSQLClause( true ) );		
		}
		else
		{
	...
	public static function processEditPageSecurity( $table )
	{
		...
		if( $pageMode != EDIT_SIMPLE )
		{
			...
			if( !isLogged() || isLoggedAsGuest() )
				$messageLink = " <a href='#' id='loginButtonContinue'>". "ログインして" . "</a>";
	...
	protected function setSuccessfulEditMessage()
	{
		...
		$this->setMessage( "<strong>&lt;&lt;&lt; "."レコードがアップデートされました。". " &gt;&gt;&gt;</strong>" );
	}
	...
	protected function checkDeniedDuplicatedValues()
	{
		...
		foreach($this->newRecordData as $f => $value)
		{
			...
			$this->errorFields[] = $f;
			$this->setMessage( $this->pSet->label( $f ) . " " . "フィールドの値が重複しています" );
			return false;
		}
		return true;
	}
	...
	protected function SecurityRedirect()
	{
		if($this->mode == EDIT_INLINE)
		{
			echo printJSON(array("success" => false, "message" => "このレコードは編集できません"));
			exit();
		}
	...
	public function setDatabaseError( $message )
	{
		if( $this->mode != EDIT_INLINE )
		{
			$this->message = "<strong>&lt;&lt;&lt; "."レコードは編集されませんでした"."</strong> &gt;&gt;&gt;<br><br>".$message;
		}
		else
			$this->message = "レコードは編集されませんでした".". ".$message;
